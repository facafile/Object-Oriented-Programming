...
public class RollingDieGUIResponsive extends JFrame  {
	...
	public class RollingTask extends SwingWorker<Void,Integer[]>{
		int noOfRolling;
		public RollingTask() {
			noOfRolling=slRollings.getValue();
		...
		protected void process(List<Integer[]> chunks) {
			...
			for(var x:chunks) {
				for(int i =0;i<6;i++) {
				txOccurences[i].setText(String.format("%d", x[i]));
				}
			}
		}
		public void done() {
